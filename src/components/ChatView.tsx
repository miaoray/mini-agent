...
export default function ChatView() {
  ...
  const userScrolledRef = useRef(false); // Use ref to keep track of the value in closures
  const {
  ...
  // Update the ref whenever userScrolled state changes
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // User is considered "scrolled away" if more than 120px from bottom
      // This allows some tolerance for minor scroll movements while still
      // detecting when user is intentionally viewing history
      const isScrolledAway = canScroll && distanceFromBottom > 120;
      
      // Update both state and ref
      setUserScrolled(isScrolledAway);
      userScrolledRef.current = isScrolledAway;
    };
  ...
  // Reset userScrolled when conversation changes (new conversation or switching)
  useEffect(() => {
    setUserScrolled(false);
    userScrolledRef.current = false;
  }, [currentConversationId]);

  // Auto-scroll when messages change (user or assistant messages)
  useEffect(() => {
    const el = scrollContainerRef.current;
    ...
    // Use double requestAnimationFrame to ensure DOM has updated with new content
  ...
  }, [isStreaming, activeThinking?.length, currentConversationId]);
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Scroll to bottom immediately when user sends a message - this is user's explicit action
      const el = scrollContainerRef.current;
